...
export async function run(argv, env = process.env, deps = {}) {
  ...
  const plan = planUploads(next, previous.manifest);

  printPlan({ ...ctx, previous, plan, files });
  if (config.dryRun) {
    log.out(`upload dry-run to-upload=${plan.changed.length} unchanged=${plan.unchanged.length} orphaned=${plan.removed.length} prev=${previous.source}`);
    return 0;
  }
  if (plan.changed.length === 0 && !plan.manifestChanged) {
    log.out(`upload ok uploaded=0 skipped=${plan.unchanged.length} removed=${plan.removed.length} bytes=0 prev=${previous.source} ms=${Date.now() - started}`);
    return 0;
  }
  ...
  if (failed.length > 0) {
    ...
    log.out(`upload failed uploaded=${results.length - failed.length} failed=${failed.length} skipped=${plan.unchanged.length} bytes=${bytes} prev=${previous.source} ms=${Date.now() - started}`);
    return EXIT_UPLOAD_FAILED;
  }

  await uploadOne({ ...ctx, key: MANIFEST_FILE, body: files.get(MANIFEST_FILE) });
  await writeCachedManifest(config.cacheDir, files.get(MANIFEST_FILE));
  log.out(`upload ok uploaded=${results.length + 1} skipped=${plan.unchanged.length} removed=${plan.removed.length} bytes=${bytes} prev=${previous.source} ms=${Date.now() - started}`);
  return 0;
}
...
/**
 * Reads every file listed in the manifest, plus manifest.json itself — per
 * docs/build-contracts.md the manifest lists "all files above", not itself, so it has to be
 * loaded separately or the final upload would have nothing to send. Warns (does not fail)
 * when a file's sha1 differs from the manifest value, which would point at a pipeline bug.
 * @param {string} outDir
...
async function readFiles(outDir, manifest, log) {
  ...
  for (const [path, sha] of Object.entries(manifest)) {
    ...
    if (path !== MANIFEST_FILE && sha1(body) !== sha) {
      log.err(`warning: ${path} content sha1 differs from manifest.json entry`);
    }
    files.set(path, body);
  }
  if (missing.length > 0) {
  ...
  if (!files.has(MANIFEST_FILE)) files.set(MANIFEST_FILE, await readFile(join(outDir, MANIFEST_FILE)));
  return files;
}
...
function printPlan({ config, client, log, previous, plan, files }) {
  ...
  log.err(`Summary: ${plan.changed.length} to upload (${formatBytes(total)}), ${plan.unchanged.length} unchanged, ${plan.removed.length} orphaned`);
}
